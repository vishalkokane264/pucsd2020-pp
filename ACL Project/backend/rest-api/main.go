...
func init() {
	...
	handlers = []handler.IHTTPHandler{
		...
		httpHandler.NewUserRoleHandler(dbConn),
		httpHandler.NewGroupsApiHandler(dbConn),
		httpHandler.NewUserPermissionHandler(dbConn), //for user_default_perm
		httpHandler.NewFileServerHandler(dbConn),     //for file_server
		httpHandler.NewFileMappingHandler(dbConn),    //for file_mapping
		httpHandler.NewUserGroupHandler(dbConn),      //for user_group
		httpHandler.NewUserFilePermHandler(dbConn),   //for user file perm
		httpHandler.NewGroupFilePermHandler(dbConn),  //for group file perm
		//		httpHandler.OtherApiHandler(dbConn),          //for other apihandling
	}
}
